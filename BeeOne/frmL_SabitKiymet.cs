...
namespace BeeOne
{
    public partial class frmL_SabitKiymet : Form
    {
        public frmL_SabitKiymet()
        {
            InitializeComponent();
            DatagridviewSetting(dgwSabitKiymetIslemleri);
        }
        ...
        {
            dgwSabitKiymetIslemleri.RowHeadersVisible = false;
            ...
            dgwSabitKiymetIslemleri.DefaultCellStyle.ForeColor = Color.White;
        ...
        private void frmL_SabitKiymet_Load(object sender, EventArgs e)
        {
            ...
            string demirbas_fis_tablo = "ONE_" + frmno + "_DEMIRBAS";
...
" ON STF.SALESMANREF = SL.LOGICALREF where STF.LOGICALREF IN(SELECT STFICHEREF FROM ONE_" + frmno + "_STLINE WHERE STOCKREF IN(SELECT LOGICALREF " +
" FROM ONE_" + frmno + "_ITEMS WHERE CARDTYPE= 4))";

            SqlDataAdapter da = new SqlDataAdapter(list_query.ToString(), baglan);
            DataSet ds = new DataSet();
            baglan.Open();
            da.Fill(ds, demirbas_fis_tablo);
            dgwSabitKiymetIslemleri.DataSource = ds.Tables[demirbas_fis_tablo];
            baglan.Close();
